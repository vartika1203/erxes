...
export class PutData<IListArgs extends IPutDataArgs>{
  ...
  private async generateStocks() {
    ...
    const categoryIds = Object.values(this.params.productsById).map(item => item['categoryId']);
    const unitedCodes = await this.models.UnitedCodes.find({ categoryId: { $in: categoryIds } });
  ...
  public async run() {
    ...
    const prePutResponse = await this.models.PutResponses.putHistories(this.models, {
    ...
    await this.models.PutResponses.updatePutResponse(this.models, resObj._id, { ...response, customerName: this.params.customerName });
...
export const returnBill = async (models, doc, config) => {
  ...
  const response = JSON.parse(responseStr);
  await models.PutResponses.updatePutResponse(models, resObj._id, { ...response });
