...
import { getFullDate, getTomorrow } from '../utils/utils';
...
const generateFilter = async (models, params, commonQuerySelector) => {
  ...
  if (params.contentType) {
    ...
    if (params.contentType === 'pos' && params.orderNumber) {
      const posOrders = await models.PosOrders.find({ number: { $regex: new RegExp(params.orderNumber) } }, { _id: 1 }).lean();
    ...
    if (params.contentType === 'deal') {
      ...
      if (params.pipelineId) {
        if (params.stageId) {
          ...
          const stages = await models.Stages.find({ pipelineId: params.pipelineId }, { _id: 1 }).lean();
